...
export function SurveyMirageDesktopLink() {
  ...
  const handleDownload = useCallback(() => {
    if (!info) {
      window.open(
        "https://github.com/loteknowledG/echo-mirage-cyberdeck/releases/download/desktop-v0.1.9/Echo-Mirage-Cyberdeck-Setup-0.1.9.exe",
        "_blank",
        "noopener,noreferrer",
      );
    ...
    >
      ...
      {info?.installerAvailable && info.fileName ? (
        ...
        <p className="text-[8px] text-[#5f5f5f]">
          Direct:{" "}
          <a
            href="https://github.com/loteknowledG/echo-mirage-cyberdeck/releases/download/desktop-v0.1.9/Echo-Mirage-Cyberdeck-Setup-0.1.9.exe"
            target="_blank"
            rel="noopener noreferrer"
            className="text-fuchsia-300/90 underline decoration-fuchsia-800/80 underline-offset-2"
          >
            Echo-Mirage-Cyberdeck-Setup-0.1.9.exe
          </a>
        </p>
      )}
      {info?.statusMessage ? (
