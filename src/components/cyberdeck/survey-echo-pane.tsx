...
export function SurveyEchoPane() {
  ...
  const handleRegenerate = useCallback(async () => {
    setBusy(true);
    setError(null);
    const status = await regenerateEchoSurveyCodes();
    setBusy(false);
    if (!status.ok) {
      setError(status.reason);
      return;
  ...
  const handleActivateCaptureRelay = useCallback(async () => {
    setRelayBusy(true);
    const handle = await startSurveyCaptureDeck();
    setRelayBusy(false);
        ...
        <>
          {miragePin || powerfistPin ? (
            <>
              ...
              />
              <PairPinBlock
                label={`CODE FOR ${SURVEY_POWERFIST_LABEL}`}
                pin={powerfistPin}
            ...
            <ul className="space-y-1">
              {pairedMirages.map((mirage) => (
                <li key={mirage.nodeId} className="text-emerald-300/80">
                  LINKED // {SURVEY_MIRAGE_DISPLAY} {mirage.nodeId.slice(0, 8)}…
                </li>
              ))}
            </ul>
          ) : (
          ...
          {pairedPowerfist ? (
            <p className="text-emerald-300/80">
              PAIRED // {SURVEY_POWERFIST_LABEL} {pairedPowerfist.deviceId.slice(0, 8)}…
            </p>
          ) : null}
          ...
          <div className="rounded border border-[#1c1c1c] bg-black/50 p-3">
            ...
            {captureRelayActive ? (
              <p className="mb-2 text-emerald-300/80">RELAY ACTIVE // capture armed</p>
            ) : (
