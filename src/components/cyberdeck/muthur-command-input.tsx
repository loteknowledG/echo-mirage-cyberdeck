...
export const MuthurCommandInput = forwardRef<MuthurCommandInputHandle, MuthurCommandInputProps>(
  function MuthurCommandInput(
    ...
    const adjustHeight = useCallback(() => {
      ...
      if (!el) return;
      el.style.height = `${Math.max(TEXTAREA_MIN_HEIGHT_PX, maxHeightPx)}px`;
    }, [maxHeightPx]);
      ...
      () => ({
        ...
        clear: () => {
          ...
          if (inputRef.current) {
            ...
            inputRef.current.style.height = `${Math.max(TEXTAREA_MIN_HEIGHT_PX, maxHeightPx)}px`;
          }
          notifyCanSend("");
        },
        element: inputRef.current,
      }),
      [adjustHeight, maxHeightPx, value],
    );
    ...
    return (
      <div className="muthur-command-input-row flex h-full min-h-0 min-w-0 flex-1 items-stretch gap-1">
        ...
        <textarea
          ...
          className="muthur-command-input h-full min-h-0 min-w-0 flex-1 resize-none overflow-y-auto rounded-none border-0 bg-black py-3 pl-1 pr-3 font-mono text-sm leading-relaxed text-green-400 placeholder:text-green-800 transition-[color,box-shadow] focus:outline-none"
          disabled={false}
