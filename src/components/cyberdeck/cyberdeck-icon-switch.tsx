...
export type CyberdeckIconSwitchProps = {
  ...
  /** Shown on the left when checked (thumb on the right — left is the inactive side). */
  leftIcon?: ReactNode;
  /** Shown on the right when unchecked (thumb on the left — right is the inactive side). */
  rightIcon?: ReactNode;
...
/** Compact switch — colored thumb on the active side, icon on the empty track side. */
export function CyberdeckIconSwitch({
  ...
  const leftSlot = icon ? (inactiveLeft ? icon : null) : inactiveLeft ? leftIcon : null;
  const rightSlot = icon ? (inactiveRight ? icon : null) : inactiveRight ? rightIcon : null;
  ...
  return (
    <SwitchPrimitives.Root
      ...
      className={cn(
        ...
        "h-5 w-[2.125rem] rounded-[2px] border border-[#2d2d2d] bg-[#090909] p-[2px]",
    ...
    >
      <span
        className={cn(
          ...
          "pointer-events-none absolute left-[2px] z-10 flex size-4 items-center justify-center text-[#8a8a8a]",
      ...
      <span
        className={cn(
          ...
          "pointer-events-none absolute right-[2px] z-10 flex size-4 items-center justify-center text-[#8a8a8a]",
      ...
      <SwitchPrimitives.Thumb
        className={cn(
          ...
          "pointer-events-none block size-4 shrink-0 rounded-[1px] border border-[#444]",
          ...
          "data-[state=checked]:translate-x-[calc(2.125rem-1rem-6px)]",
