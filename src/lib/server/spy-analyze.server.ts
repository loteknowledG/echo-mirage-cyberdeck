// SERVER ONLY — vision analysis for Spy / Espionage captures.
...
function resolveVisionModel(provider: string): string {
  const fromEnv = process.env.SPY_VISION_MODEL?.trim();
...
export async function analyzeSpyCapture(input: SpyAnalyzeInput): Promise<SpyAnalyzeResult> {
  ...
  if (!endpoint) {
    return { ok: false, error: `Provider "${provider}" does not support Spy vision yet. Use openai or openrouter.` };
  }
  ...
  const prompt = input.prompt?.trim() || DEFAULT_SPY_PROMPT;
